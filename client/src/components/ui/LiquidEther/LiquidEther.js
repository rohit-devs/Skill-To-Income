// ----------------------------------------------------------------------
// File: client/src/components/ui/LiquidEther/LiquidEther.js
...
`;
    ...
    try {
      ...
      console.warn('Failed to construct WebGLManager:', err);
    ...
    try {
      ...
      console.warn('Failed to start WebGLManager:', err);
      setWebglSupported(false);
      return;
    }
    ...
    return () => {
      ...
      if (webglRef.current) {
        try {
          ...
          console.warn('Failed to dispose LiquidEther:', e);
        }
      }
  ...
  useEffect(() => {
    if (!webglSupported || !isWebGLAvailable()) return;
  ...
  if (!webglSupported || !isWebGLAvailable()) {
    return <div ref={mountRef} className={`liquid-ether-container ${className || ''}`} style={{ ...style, background: '#0B0F1A' }} />;
  }
