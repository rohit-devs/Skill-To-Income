// ----------------------------------------------------------------------
// File: client/src/components/ui/Hyperspeed/Hyperspeed.js
...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      myApp.loadAssets().then(myApp.init).catch(err => {
        console.warn('Failed to load assets or init Hyperspeed:', err);
        setWebglSupported(false);
      });
    } catch (err) {
      console.warn('Failed to initialize Hyperspeed WebGL:', err);
      setWebglSupported(false);
    }
    ...
    return () => {
      if (appRef.current) {
        try {
          ...
          console.warn('Failed to dispose Hyperspeed:', e);
        }
        appRef.current = null;
      }
    };
  ...
  if (!webglSupported || !isWebGLAvailable()) {
    return <div id="lights" ref={hyperspeed} style={{ background: '#06080F', width: '100%', height: '100%' }}></div>;
  }
