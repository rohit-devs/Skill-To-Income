...
router.get('/earnings', protect, async (req, res) => {
  try {
    const completedTasks = await Task.find({ assignedTo: req.user._id, status: 'completed' })
      .populate('postedBy', 'name businessName')
      .sort({ updatedAt: -1 });

    const totalEarned = completedTasks.reduce((sum, t) => sum + (t.studentPay || 0), 0);
...
router.get('/leaderboard', async (req, res) => {
  try {
    const leaders = await User.find({ role: 'student', tasksCompleted: { $gt: 0 } })
      .select('name college city totalEarned tasksCompleted rating isSenior skills')
      .sort({ totalEarned: -1 })
      .limit(10);
    res.json(leaders);
  } catch (err) { res.status(500).json({ message: err.message }); }
...
router.get('/:id', async (req, res) => {
  try {
    const user = await User.findById(req.params.id).select('-password -email -whatsapp');
    if (!user) return res.status(404).json({ message: 'User not found' });
    const recentTasks = await Task.find({ assignedTo: req.params.id, status: 'completed' })
      .select('title category budget updatedAt').limit(5);
