...
async function seed() {
  ...
  const inserted = await users.insertMany([
    ...
    { name:'Ravi Kulkarni', email:'business@demo.com', password:hash,
      ...
      isSenior:false, tasksCompleted:0, tasksPosted:6, totalEarned:0,
      isVerified:true, isBanned:false, createdAt:new Date() },
    { name:'TechStart HR',  email:'company@demo.com',  password:hash,
      role:'company',  businessName:'TechStart India', city:'Mumbai',
      isSenior:false, tasksCompleted:0, tasksPosted:1, totalEarned:0,
      isVerified:true, isBanned:false, createdAt:new Date() },
  ...
  const biz = inserted.insertedIds[2];
  const company = inserted.insertedIds[3];
  const now = new Date();
  ...
  await tasks.insertMany([
    ...
    { title:'Social media content calendar for November',
      ...
      postedBy:biz, assignedTo:null, isPaid:false, createdAt:now },
    { title:'Create a hiring announcement banner',
      description:'Design a clean 1200x628 hiring banner for TechStart India with role title, remote-first note, and QR code placeholder. Deliver PNG plus editable design link.',
      category:'Design', budget:320, deadline:'24 hrs', revisions:1,
      priority:false, status:'open', commission:0.12, studentPay:282,
      postedBy:company, assignedTo:null, isPaid:false, createdAt:now },
  ]);
